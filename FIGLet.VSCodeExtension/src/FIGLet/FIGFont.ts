...
export class FIGFont {
     ...
     */
    public static async fromFile(path: string): Promise<FIGFont> {
        if (!path) {
     ...
     */
    public static fromText(text: string): FIGFont {
        if (!text) {
     ...
     */
    public static fromLines(lines: string[]): FIGFont {
    ...
    private static async loadDefaultFont(): Promise<FIGFont | null> {
        try {
            // In a VSCode extension, we'll use the extension's context to locate resources
            const fontPath = path.join(__dirname, '..', '..', 'resources', 'fonts', 'small.flf');
            console.log('Loading default font from:', fontPath);
